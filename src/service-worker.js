...
self.addEventListener("push", function (event) {
  ...
  if (mLastPush.changeMessage) {
    const notificationPromise = getConstants().then((constants) => {
      ...
      if (constants && constants.notification_icon)
        options.icon = constants.notification_icon;
      ...
      .then(async function (clientList) {
        ...
        if (pwaSupported) {
          for (var i = 0; i < clientList.length; i++) {
            ...
            if (
              client.url.indexOf("homescreen=1") != -1 &&
              "focus" in client &&
              !isCustomLink
            ) {
              return client.focus();
...
async function handlePushTokenChange(event) {
  ...
  let response = await fetch(
    `${constants.domain}/api/passes/tracker/${tracker_cached}/updatetoken/`,
    {
      method: "POST",
      body: JSON.stringify({
...
async function fetchNotifications() {
  ...
  let constants = await getConstants();
  var url =
    "{{domain}}/api/communication/notification/?tracker={{tracker.id}}&order_by=-createdAt";
  ...
  return fetch(url)
    ...
    .then((data) => {
      ...
      if (data.results.length) {
        ...
        return localforage
          .setItem(NOTIF_DB, formatted_notifs)
          .then(function () {
            broadcastMessage({
              refresh: true,
            });
...
function isPwaSupported() {
  var browser_list =
    PWA_SUPPORTED_BROWSERS[bowser.mobile ? "mobile" : "desktop"];

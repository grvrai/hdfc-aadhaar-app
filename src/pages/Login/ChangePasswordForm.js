...
export default function ChangePasswordForm({}) {
	...
	const handleSubmit = async function (e) {
		...
		try {
			await fetchData("PUT", `${Constants.domain}/api/update_password/?self=true`, {
	...
	return (
		<FormContainer>
			{isSuccess ? (
				...
				<form className="password-reset-req-form" autoComplete="off" onSubmit={handleSubmit}>
					<Grid container spacing={3}>
						...
						<Grid item xs={12}>
							<TextField
								...
								helperText={formError.old_password ? <div>{formError.old_password.join("<br/>")}</div> : ""}
								value={old_password}
